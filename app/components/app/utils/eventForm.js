(function() {
    ...
    var app$utils$eventForm$$mode = document.getElementById('reg_mode');
    app$utils$eventForm$$mode.addEventListener('click', function() {
        for (var x in app$utils$eventForm$$mode.children) {
            if (x<app$utils$eventForm$$mode.children.length) {
                if (app$utils$eventForm$$mode.children[x].checked) {
            ...
            if (xhr.readyState === 4){
                if (xhr.status === 200) {				
                    ...
                    if (json.error == false) {
                        ...
                        xt.send(JSON.stringify(app$utils$eventForm$$send))
                    }
                }
            }
        }
