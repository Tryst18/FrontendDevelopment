(function() {
    ...
    function app$utils$forgot$$onSubmit() {
        // var url = "http://localhost:4000/"
        var xhr = new XMLHttpRequest();
        xhr.open("POST", $$index$$url+"/api/user/forgot", true);
            ...
            if (xhr.readyState === 4){
                if (xhr.status === 200) {
                    ...
                    if (json.error == true) {
                        ...
                        document.getElementById('userInfo').innerHTML = '<p>Check your email for password reset link</p>'
    ...
    document.getElementById('submit').addEventListener('click', app$utils$forgot$$onSubmit);

    document.onkeyup = function(e) {
        var key = e.keyCode ? e.keyCode : e.which;
        if (key == 13) {
            app$utils$forgot$$onSubmit();
        }
    }
}).call(this);