...
document.getElementById('launch').addEventListener('click', function() {
    ...
    if (!pocE) {send = Object.assign(send, {"poc":[poc]})}
    if (!dtvE) {send = Object.assign(send, {"dtv":[dtv]})}
    if (document.getElementById('phot').value!="") {send = Object.assign(send, {"photos":[document.getElementById('phot').value]})}
    // send = Object.assign(send, {"reg_deadline":(new Date(Date.parse(document.getElementById('dead').value)).toISOString())})
    if (document.getElementById('sub').value) {send = Object.assign(send, {"subheading":document.getElementById('sub').value})}
    if (document.getElementById('place').value) {send = Object.assign(send, {"rules":document.getElementById('place').value})}
    console.log(send)
	...
	// 			var data = json.data;
				...
				// if (json.error == false) {
                    ...
                    xt.open("POST", url+"/api/event/modify/"+document.location.search.split('?')[1], true)
